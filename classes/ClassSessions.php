...
class ClassSessions{
    ...
    public function setSessions($email)
    {
        ...
        $_SESSION["login"]=true;
        $_SESSION["time"]=time();
        $_SESSION["name"]=$this->login->getDataUser($email)['data']['nomeUser'];
        $_SESSION["email"]=$this->login->getDataUser($email)['data']['emailUser'];     
        $_SESSION["permition"]=$this->login->getDataUser($email)['data']['permissaoUser'];      
        ...
        if(!isset($_SESSION['login']) || !isset($_SESSION['permition']) || !isset($_SESSION['canary'])){
            ...
            echo"
                <script>
                    ...
                    window.location.href='".DIRPAGE."views/login.php';
                </script>
            ";
        }else{
            if($_SESSION['time'] >= time() - $this->timeSession){
                ...
                if($_SESSION['permition'] !='manager' && !$permition != $_SESSION['permition']){
                    echo"
                    <script>
                     alert('Você não tem acesso a esse conteúdo.');
